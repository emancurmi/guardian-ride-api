...
describe('User Guardian Endpoints', () => {
        ...
        it('adds a new user guardian to the store', () => {
            ...
            return supertest(app)
                ...
                .expect(res => {
                    ...
                    expect(res.body).to.have.property('userguardianid')
                    expect(res.headers.location).to.eql(`/api/user_guardian/${res.body.userguardianid}`)
                })
                .then(res =>
                    supertest(app)
                        .get(`/api/user_guardian/${res.body.userguardianid}`)
                        .set('Authorization', `Bearer ${process.env.API_TOKEN}`)
                        .expect(res.body)
                )
        })
    ...
    describe('GET /api/user_guardian/:userguardianid', () => {
        context(`Given no User Guardian`, () => {
        ...
        context('Given there are user guardian links in the database', () => {
            ...
            it('responds with 200 and the specified user guardian link', () => {
                const userguardianId = 2
                const expectedUserGuardian = restultUserGuardians[userguardianId - 1]
                return supertest(app)
                    .get(`/api/user_guardian/${userguardianId}`)
                    .set('Authorization', `Bearer ${process.env.API_TOKEN}`)
    ...
    describe('DELETE /api/user_guardian/:userguardianid', () => {
        ...
        context('Given there are user guardian links in the database', () => {
            ...
            it('removes the drink by ID from the store', () => {
                const idToRemove = 2
                const expectedUserGuardian = resultsUserGuardians.filter(userguardian => userguardian.userguardianid !== idToRemove)
                return supertest(app)
